...
### 短命 JVM 加 `-Xint`：值得，而且效果很大 (2026-08-30)

`-Xint` 完全关掉 JIT，只用解释器。在 runc 上这是纯亏本；在沙箱里 JIT 编译本身
（编译线程的 mmap、代码缓存缺页、跨线程同步）恰好是最贵的部分，所以对**跑完就退出**
的 JVM 反而是赚的。中位数取 3 次：

```
                     runc                      systrap                   ptrace
              默认   C1only  -Xint      默认   C1only  -Xint      默认   C1only  -Xint
java Hello    0.50   0.51    0.51       2.90   2.05    1.53       2.96   2.21    1.76
javac 一个文件 0.71   0.69    0.76       7.79   5.92    1.80       8.55   5.88    1.95
```

相对 runc：`javac` 从 **11.0x 降到 2.4x**，`java Hello` 从 5.8x 降到 3.0x。
runc 侧几乎没有区别（0.50/0.51/0.51），所以这纯粹是沙箱侧的收益。

规模也不改变结论。用一批带 lambda / stream / 方法引用的生成源码（会走 invokedynamic
的 LambdaMetafactory，类加载和 JIT 都很重）：

```
javac N 个文件        runc                      systrap
                默认   C1only  -Xint      默认   C1only  -Xint
  N=1           1.54   1.36    3.81      29.93  25.75   10.08
  N=10          1.73   1.56    4.14      40.04  33.91   10.82
  N=40          2.28   1.92    5.38      49.09  55.33   12.75
  N=80          2.78   2.30    6.97      44.43  62.03   15.87
```

到 80 个文件（十几秒）`-Xint` 仍然大幅领先。注意 `C1only` 在 N≥40 时**比默认还慢**
（55.33 / 62.03），因为 C1 不做基于 profile 的筛选，会把所有方法都编译一遍。

顺带暴露一件事：**invokedynamic / lambda 的引导在沙箱里特别贵**。同样是编译一个文件，
不带 lambda 的 `JBench.java` 是 7.79s，带 lambda/stream 的生成文件是 29.93s；
runc 侧只从 0.71s 涨到 1.54s。

**但 `-Xint` 只对"没有热循环"的负载成立。** 同一个 JVM 里跑一遍纯数值热循环：

```
Compute（1 遍热循环）  runc          systrap
默认                    1.21s         4.85s
C1only                  2.11s         4.14s
-Xint                   7.59s         8.69s   ← 解释器执行热循环，反而最慢
```
